...
def pnp_deblurring(
    ...
    Phi = np.random.rand(*x_observed[0].shape, seed=42) < prop_masked
    Phit = Phi
    gamma = 1.0
...
def generate_results_pnp(prop_masked,
                         std_noise,
                         DENOISERS,
                         img,
                         n_iter_per_lambda=10,
    ...
    Phi = np.random.rand(*img[0].shape) < prop_masked
    Phit = Phi
...
def main():
    ...
    parser.add_argument("-prop_mask", type=float, default=config.get("prop_mask", 0.7))
    ...
    for i in args.img_list:
        ...
        results = generate_results_pnp(args.prop_mask,
                                       args.std_noise,
                                       DENOISERS,
                                       img,
